...
#if defined(NDEBUG)
# error "Nitoneum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x82315d435eabf7a25b4b312df36281b745b3caaccd7fc7cfce19a02c3ddb1fc8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nitoneum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nitoneum Signed Message:\n";
...
{
    // Nitoneum: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1680000); // Nitoneum: 1.68M blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Nitoneum: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Nitoneum: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nitoneum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nitoneum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa593be3dd532f35e8ec09366b2515318ef7749137bcf329e2d18c97f848d5330");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "On May 3rd 2021, niton and NeoM mined the Genesis block, nitoneum was born.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040cc6eea720beabc7ddf125eac78bccf4fb0266d065387bc6d04e0ea07fdd598ee227fe9f48597b5fc6c2228f15af809f97f93afe418f1041ea5dd6e15744b13f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1619998918;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085290962;

        if (fTestNet)
        {
            block.nTime    = 1619998880;
            block.nNonce   = 385618956;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcfe47d22e0e2d14ab23cc29ff5bbc0af00ef4e7ba107b8a9011e25b759b6d4c0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x4b, 0x75, 0x64, 0x6f }; // Nitoneum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NitoneumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NitoneumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NitoneumMiner(CWallet *pwallet)
{
    printf("NitoneumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nitoneum-miner");
    ...
    try { loop {
        ...
        printf("Running NitoneumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NitoneumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NitoneumMiner, pwallet));
}
